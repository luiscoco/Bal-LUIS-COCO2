...
namespace Dalssoft.DiagramNet
{
	...
	/// </summary>
	internal class CalentadorResultadosController: RectangleController, IController
	{
		public CalentadorResultadosController(BaseElement element): base(element)
		{
		}
		...
		public override void DrawSelection(System.Drawing.Graphics g)
		{
            ...
            if ((el.Location.X > 0) && (el.Location.Y > 0) && (el.Size.Width > 0) && (el.Size.Height > 0))
            {
                ...
                puntos1[0].Y = el.Location.Y + 3 * el.Size.Height / 10;
                puntos1[1].X = el.Location.X + el.Size.Width / 2;
                puntos1[1].Y = el.Location.Y + 2 * el.Size.Height / 10;
                puntos1[2].X = el.Location.X + el.Size.Width / 2;
                puntos1[2].Y = el.Location.Y + 4 * el.Size.Height / 10;
                puntos1[3].X = el.Location.X + el.Size.Width;
                puntos1[3].Y = el.Location.Y + 3 * el.Size.Height / 10;
                g.DrawLines(p1, puntos1);

                Point[] puntos2 = new Point[2];
                puntos2[0].X = el.Location.X;
                puntos2[0].Y = el.Location.Y + 8 * el.Size.Height / 10;
                puntos2[1].X = el.Location.X + el.Size.Width;
                puntos2[1].Y = el.Location.Y + 8 * el.Size.Height / 10;
