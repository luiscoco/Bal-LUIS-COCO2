...
{
    public partial class Turbina10 : Form
    {
        Double D1,D2, D3, D4, D5, D8;
        ...
        public Turbina10(Aplicacion punteroaplicion, Double numecuaciones1, Double numvariables1)
        {
            InitializeComponent();
        ...
        private List<String> generaecucaiones(Double D1,Double D2, Double D3, Double D4, Double D5, Double D8, Double correntrada, Double corrsalida)
        {
            ...
            if (D3 != 0)
            {
                ...
                punteroaplicacion1.p[(int)numparametroscreados].Value = (D3 * (Math.Sqrt(D2 / (punteroaplicacion1.acceso.vph(D2, punteroaplicacion1.p.Find(p => p.Nombre == "H" + Convert.ToString(correntrada)).Value)))));
                punteroaplicacion1.p.Find(p => p.Nombre == "W" + Convert.ToString(correntrada)).Value = (D3 * (Math.Sqrt(D2 / (punteroaplicacion1.acceso.vph(D2, punteroaplicacion1.p.Find(p => p.Nombre == "H" + Convert.ToString(correntrada)).Value)))));
            }
            else if (D3 == 0)
            {
                punteroaplicacion1.p[(int)numparametroscreados].Nombre = "W" + Convert.ToString(corrsalida);
