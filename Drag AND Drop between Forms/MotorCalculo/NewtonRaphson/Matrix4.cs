...
namespace NumericalMethods.FourthBlog
{
    public class Matrix
    {
        ...
        {
            //Confirmamos que el número de filas de la matriz b (rightMatrix) es igual al número de columnas de la Matriz A
            Debug.Assert(rightMatrix.RowCount == _columnCount);

            //Confirmamos que el número de filas y columnas de la Matriz A son iguales
            Debug.Assert(_columnCount == _rowCount);

            //Creamos un Objeto para guardar la Matriz de Resultados
            Matrix resultMatrix = new Matrix(_columnCount, rightMatrix.ColumnCount);

            //Realizamos la Descomposición LU de A
            LUDecompositionResults resDecomp = LUDecompose();

            //Confirmamos que durante la Descomposción LU de A no se ha producido un error por MATRIZ SINGULAR
            if (resDecomp.errorluiscoco == "matriz singular")
            {
                resultMatrix[0, 0] = 999999999999;
                return resultMatrix;
            }

            //Gardamos la Matriz de PERMUTACIONES P en el array nP
            int[] nP = resDecomp.PivotArray;
            //Guardamos la Matriz L en la matriz lMatrix
            Matrix lMatrix = resDecomp.L;
            //Guardamos la Matriz U en la matriz uMatrix
            ...
            for (int k = 0; k < rightMatrix.ColumnCount; k++)
            {
                ...
                //Cálculo de la Matriz d (dMatrix)
                double sum = 0.0;
                ...
                //Cálculo de la matriz x (resultMatrix)
        ...
        //Cálculo de la Descomposición LU de A mediante el Método de CROUT con la matriz de permitaciones P
            ...
            try
            {
                ...
                for (int i = 0; i < _rowCount; i++)
                {
                    ...
                    for (int j = i; j < _rowCount; j++)
                    {
                        ...
                        if (rowSum == 0.0)
                        {
                            result.errorluiscoco = "matriz singular";
                            MessageBox.Show("ERROR LUIS COCO MATRIZ SINGULAR.");
                            return result;
                ...
                //Resultados de la descomposicion LU de A son las matrices L(lMatrix), U(uMatrix) y P(pivotArray)
                result.U = uMatrix;
                result.L = lMatrix;
